...
/**
 * Flagship "systems story": a cinematic mission-control sequence fusing
 * Input → Logic → Dashboard → Result with Problem → Architecture → Product.
 * Desktop pins the stage and advances it with scroll; mobile and
 * prefers-reduced-motion get a premium stacked, non-pinned version.
  ...
  return (
    ...
    >
      <div className="w-full overflow-hidden rounded-lg border border-border/70 bg-background/40">
        ...
        <div className="flex">
          <div className="hidden w-16 shrink-0 flex-col gap-2 border-r border-border/60 p-3 sm:flex">
      ...
      <div className="flex w-full max-w-[430px] items-center gap-2">
        {parts.map((p, i) => (
          <div key={p} className="contents">
            {i > 0 && (
              <div className="relative h-px flex-1 bg-gradient-to-r from-border via-accent/50 to-border">
                <motion.span
            ...
            <div className="rounded-lg border border-accent/25 bg-card px-4 py-3 text-center">
        ...
        <Container className="relative w-full">
          <div className="flex items-end justify-between gap-6">
            ...
            <p className="shrink-0 font-mono text-sm text-muted-foreground">
              <span className="text-accent">{STAGES[stage].id}</span>
          ...
          <div className="mt-12 grid grid-cols-[190px_minmax(0,1fr)_300px] items-center gap-12">
                ...
                {STAGES.map((s, i) => (
                  <li key={s.id} className="relative pl-7">
                    <span
                      aria-hidden
                      className={cn(
                        "absolute top-1.5 left-0 size-[11px] rounded-full border transition-colors duration-500",
                        i <= stage
                    ...
                    <p
                      className={cn(
                        ...
                        i === stage ? "text-foreground" : "text-muted-foreground/70",
                      )}
                    >
                      {s.id} {s.title}
                    </p>
            ...
            <div className="relative aspect-[16/9] overflow-hidden rounded-xl border border-border bg-card/50 ring-1 ring-white/5 backdrop-blur-sm">
              ...
              {STAGES.map((s, i) => (
                <motion.div
                  ...
                  animate={{
                    opacity: i === stage ? 1 : 0,
                    y: i === stage ? 0 : i < stage ? -26 : 26,
                    scale: i === stage ? 1 : 0.97,
                  }}
            ...
            <div>
              {STAGES.map((s, i) => (
                <motion.div
                  key={s.id}
                  className={cn(i === stage ? "block" : "hidden")}
                  initial={false}
                  animate={{ opacity: i === stage ? 1 : 0 }}
        ...
        {STAGES.map((s, i) => (
          <Reveal key={s.id} delay={0.04}>
            <div className="overflow-hidden rounded-xl border border-border bg-card/60 ring-1 ring-white/5">
              ...
              <div className="relative h-44 border-b border-border/60 p-4">
