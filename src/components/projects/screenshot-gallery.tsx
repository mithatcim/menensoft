...
export function ScreenshotGallery({
  ...
  const step = useCallback(
    (d: number) => {
      ...
      setOpen((open + d + items.length) % items.length);
    },
    [open, items],
  );
    ...
    <section className="border-t border-border/60 py-16 md:py-20">
      ...
      {active && (
        <div
          ...
          className="fixed inset-0 z-[120] flex flex-col bg-foreground/60 backdrop-blur-sm"
        ...
        >
          ...
          >
            <button
              ...
              onClick={() => step(-1)}
              className="absolute left-2 z-10 rounded-full bg-background/90 p-2 text-foreground shadow-md transition-colors hover:bg-background md:left-6"
            >
              <ChevronLeft className="size-5" />
            </button>
            {/* plain img: lightbox at arbitrary viewport sizes, already local PNG */}
            ...
            <button
              ...
              onClick={() => step(1)}
              className="absolute right-2 z-10 rounded-full bg-background/90 p-2 text-foreground shadow-md transition-colors hover:bg-background md:right-6"
            >
